...
<script>
	...
	%>
		...
		myModal.modal();
		$('#inputEmail').val('<%=request.getAttribute("errorId") %>');
		$('#inputPassword').val('<%=request.getAttribute("errorPwd") %>');
		$('#inputName').val('<%=request.getAttribute("errorName") %>');
		$('#inputNickName').val('<%=request.getAttribute("errorNickname") %>');
	<% 
		}
	%>
</script>
